...
function AdminLoginForm({ onLogin }: { onLogin: () => void }) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setLoading(true);
    try {
      console.log("Admin: Credential check passed, logging in...");
      try {
        await signInWithEmailAndPassword(auth, email.trim(), password);
        console.log("Admin: Firebase Auth sign-in successful");
      } catch (firebaseErr: any) {
        console.log("Admin: Firebase Auth failed (non-blocking):", firebaseErr?.code);
        if (firebaseErr?.code === "auth/user-not-found") {
          try {
            await createUserWithEmailAndPassword(auth, email.trim(), password);
            console.log("Admin: Firebase account created");
          } catch (createErr: any) {
            console.log("Admin: Firebase create also failed (non-blocking):", createErr?.code);
          }
        }
      }
      if (Platform.OS !== "web") {
      ...
      console.log("Admin login unexpected error:", err?.message);
      if (Platform.OS !== "web") {
        void Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      }
      setError("");
